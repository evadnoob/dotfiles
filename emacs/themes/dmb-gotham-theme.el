...
(defgroup dmb-gotham nil
  "A very dark Emacs theme"
  :prefix "dmb-gotham-")

(deftheme dmb-gotham "The Dmb-Gotham color theme")

(defcustom dmb-gotham-tty-extended-palette nil
...
(defvar dmb-gotham-color-alist
  `((base0   "#0c1014" ,(if dmb-gotham-tty-extended-palette "color-232" "color-16"))
    (base1   "#11151c" ,(if dmb-gotham-tty-extended-palette "color-233" "brightblack"))
    (base2   "#091f2e" ,(if dmb-gotham-tty-extended-palette "color-17"  "brightgreen"))
    (base3   "#0a3749" ,(if dmb-gotham-tty-extended-palette "color-18"  "brightblue"))
    (base4   "#245361" ,(if dmb-gotham-tty-extended-palette "color-24"  "brightyellow"))
    (base5   "#599cab" ,(if dmb-gotham-tty-extended-palette "color-81"  "brightcyan"))
    (base6   "#99d1ce" ,(if dmb-gotham-tty-extended-palette "color-122" "white"))
    (base7   "#d3ebe9" ,(if dmb-gotham-tty-extended-palette "color-194" "brightwhite"))
...
See `dmb-gotham-transform-face' for the transformation, see
`dmb-gotham-transform-spec' for the rules."
  (apply #'custom-theme-set-faces 'dmb-gotham
         (mapcar #'dmb-gotham-transform-face faces)))

(defun dmb-gotham-transform-face (face)
...
graphical and terminal displays.  See `dmb-gotham-transform-spec' for
the rules that are applied to the face attributes."
  (let* ((name (car face))
         (spec (cdr face))
         (graphic-spec (dmb-gotham-transform-spec spec 'graphic))
         (tty-spec (dmb-gotham-transform-spec spec 'tty)))
    `(,name ((((type graphic)) ,@graphic-spec)
             (((type tty)) ,@tty-spec)))))

(defun dmb-gotham-transform-spec (spec display)
  "Helper function that transforms SPEC for DISPLAY.
DISPLAY is either 'graphic or 'tty, SPEC is a property list where
the values are substituted with colors from `dmb-gotham-color-alist'
...
(provide-theme 'dmb-gotham)

;;; dmb-gotham-theme.el ends here
